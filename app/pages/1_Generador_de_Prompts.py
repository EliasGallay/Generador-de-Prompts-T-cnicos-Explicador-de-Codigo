# app/pages/1_Generador_de_Prompts.py
...
def render():
    ...
    # Campo principal de tarea (usa session_state si se tocó un ejemplo)
    tarea = st.text_area(
        "¿Qué necesitás hacer?",
    ...
    with st.form("form_prompt"):
        ...
        with col1:
            ...
            lenguaje = st.selectbox("Lenguaje principal", ["(no especificar)", "JavaScript/TypeScript", "Python", "Java", "SQL", "Go", "Otro"])
            if lenguaje == "(no especificar)":
                lenguaje = None
        with col2:
            stack = st.multiselect("Stack/Frameworks (opcional)", ["Node.js", "Express", "FastAPI", "Django", "React", "Next.js", "NestJS", "PostgreSQL", "MongoDB", "Docker", "PyTest", "Jest"])
            tono = st.selectbox("Tono (opcional)", ["", "didáctico", "conciso", "paso a paso", "estricto"])

        contexto = st.text_area("Contexto (opcional)", placeholder="Ej: Proyecto monorepo, CI con GitHub Actions, deploy en Railway…", height=100)
        ...
        submit = st.form_submit_button("Generar prompt", use_container_width=True, type="primary")
    ...
    if submit:
        ...
        restricciones_list = [l.strip("- ").strip() for l in (restricciones.splitlines() if restricciones else []) if l.strip()]
        salida_list = [l.strip() for l in (salida.splitlines() if salida else []) if l.strip()]
